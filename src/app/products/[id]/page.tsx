...
export default function ProductDetailPage() {
  ...
  const afterBalance = balance - product.price * qty;
        ...
        <div className="max-w-[1070px] mx-auto">
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="flex-1">
              <div className="text-xs text-[#CC0000] font-bold tracking-[2px] uppercase mb-2">{product.brand}</div>
              ...
              <div className="rounded-xl p-5 mb-5" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
                <div className="text-[28px] font-black text-[#CC0000]">{formatPrice(product.price)}</div>
                <div className="flex items-center gap-3 mt-3 pt-3 text-[13px]" style={{ borderTop: "1px solid var(--border)", color: "var(--text-sec)" }}>
                  내 잔액: <strong className="text-[#03C75A]">{formatPrice(balance)}</strong>
                  &nbsp;|&nbsp; 구매 후 잔액: {formatPrice(afterBalance)}
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => { addToCart(product, qty); router.push("/cart"); }}
                  className="flex-1 bg-[#CC0000] text-white text-base font-bold py-4 rounded-xl hover:bg-[#e00] transition-all">
                  바로 구매
                </button>
                <button onClick={() => addToCart(product, qty)}
                  className="px-6 py-4 rounded-xl text-base font-semibold flex items-center gap-2 transition-all hover:border-[rgba(204,0,0,0.3)]"
