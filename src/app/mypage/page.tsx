      ...
      <main className="flex-1 px-6 py-8">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>OO정비소님 환영합니다</p>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                {[
                  { val: "324,000", label: "잔액 (원)", link: "/charge" },
                  { val: "12", label: "총 주문", link: "#" },
                  { val: "2", label: "배송 중", link: "#" },
                ].map((s) => (
                  <Link key={s.label} href={s.link}
                    className="rounded-xl p-5 text-center transition-all hover:border-[rgba(204,0,0,0.3)]"
                    style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
                    <div className="text-2xl font-black text-[#CC0000]">{s.val}</div>
              ...
              <div className="rounded-xl overflow-hidden mb-6" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
                {orders.map((o, i) => (
                  <div key={i} className="flex justify-between px-5 py-4 text-[13px]"
